...
public class AtomixAsyncMap<K, V> implements AsyncMap<K, V> {
  ...
  AtomixAsyncMap(DistributedMap<K, V> map) {
    this.map = Assert.notNull(map, "map");
  }

  @Override
  public void get(K k, Handler<AsyncResult<V>> handler) {
    map.get(k).whenComplete(VertxFutures.resultHandler(handler));
  }
  ...
  public void put(K k, V v, long l, Handler<AsyncResult<Void>> handler) {
    map.put(k, v, Duration.ofMillis(l)).whenComplete(VertxFutures.voidHandler(handler));
  }

  @Override
  public void putIfAbsent(K k, V v, Handler<AsyncResult<V>> handler) {
    map.putIfAbsent(k, v).whenComplete(VertxFutures.resultHandler(handler));
  }

  @Override
  public void putIfAbsent(K k, V v, long l, Handler<AsyncResult<V>> handler) {
    map.putIfAbsent(k, v, Duration.ofMillis(l)).whenComplete(VertxFutures.resultHandler(handler));
  }

  @Override
  public void remove(K k, Handler<AsyncResult<V>> handler) {
    map.remove(k).whenComplete(VertxFutures.resultHandler(handler));
  }
  ...
  public void clear(Handler<AsyncResult<Void>> handler) {
    map.clear().whenComplete(VertxFutures.voidHandler(handler));
  }

  @Override
  public void size(Handler<AsyncResult<Integer>> handler) {
    map.size().whenComplete(VertxFutures.resultHandler(handler));
  }
